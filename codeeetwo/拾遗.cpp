...
//tip abs()意思是算绝对值
/* stack栈，先进后出，后进先出
 格式：stack<int> s; //声明存储int类型数据的栈
   s.push(1） ； //{}变成 {1}
s.push(2) ; //{1}变成{1,2}
printf("%d\n", s.top()) // print2
s.pop();//变成{1}
s.pop();//变成{}    */
//queue队，先进先出，格式除将stack改queue一摸一样
/*对于if(!a)来说，要看你给a的初值是什么，如果是一个非零值的话，那么!a就是假，不执行语句；
如果a的初值为零，那么!a就是真，执行语句（另外，C++中把非零的数看作是真，零为假）
*/
...
/*二叉堆 ,优先队列
 进阶P92，讲挺清楚的
 c++ stl中含有priority_queue <类型> q;指最大堆；
 支持push(上面的insert),top,pop操作；
...
*/
/*hash方法可代替log运算
//tip 因为cmathlog运算复杂度大……
//tip 法一：n较小时：建立数组H，H【2^k]=k
实现：cont max_n =1<<20;
int H[max_n+1];
for(int i=0;i<=20;i++)H[1<<i]=i;//预处理
while(cin>>n)
{
    while(n>0){cout<<H[n&-n]<<' '
    n-=n&-n;}
    cout<<endl
}
法2：（效率更高）：建立长位37的数组（2是37的原根）H，令H[2^kmod37]=k;
实现：int H[37];
 ...
 for(int j=0;j<=m;j++)
  ...
  !任选一个位置断开，复制形成2倍长度的链。
   ...
   个人以为就是记忆化搜索，即把之前dfs return的内容用dp数组储存即可了;
    ...
    状态压缩：集合的状态区与数对应，可以采用进制，

    数位dp:特点：给一些限制条件，求第k小数或区间内有多少满足限制条件的数
...
/* 并查集：
 ...
 维护fa[x]:保存x的父节点。
 操作：get:get to know x belongs to which group
